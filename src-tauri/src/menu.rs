...
/// Tauri's standard menu with the two update items folded in: on macOS they sit
/// in the application menu under About, everywhere else at the top of Help.
pub fn build(app: &AppHandle) -> tauri::Result<Menu<Wry>> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if let Some(app_menu) = items.first().and_then(|item| item.as_submenu()) {
            app_menu.insert_items(&[&separator, &check_now, &auto_check], 1)?;
        }
    }
